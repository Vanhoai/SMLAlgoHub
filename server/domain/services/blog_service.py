...
from server.domain.usecases.blog_usecases import (
    CreateBlogReq,
    FindBlogWithOptionsQuery,
    ManageBlogUseCase,
    UpdateBlogReq,
)
...
class BlogService(ManageBlogUseCase):
    ...
    async def create_blog(self, req: CreateBlogReq) -> BlogEntity:
        ...
        for i in range(len(req.tags)):
            if not ObjectId.is_valid(req.tags[i]):
                raise ExceptionHandler(
                    code=ErrorCodes.BAD_REQUEST, msg=f"Tag {req.tags[i]} is not valid"
                )
    ...
    async def find_blog_by_id(self, id: string) -> Optional[BlogEntity]:
        if not ObjectId.is_valid(id):
            raise ExceptionHandler(
                code=ErrorCodes.BAD_REQUEST, msg="Please provide a valid blog id"
            )

        return await self.blog_repository.find_one({"id": ObjectId(id)})

    async def find_blogs_by_options(
        self, query: FindBlogWithOptionsQuery
    ) -> Tuple[List[BlogEntity], Meta]:
        pipeline = []
        match_conditions = {}
        ...
        response = await self.blog_repository.find_pagination(
