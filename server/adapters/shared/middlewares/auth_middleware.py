...
async def require_token(req: Request) -> OAuthClaims:
    if not req.headers["Authorization"]:
        raise ExceptionHandler(
            code=ErrorCodes.UNAUTHORIZED, msg="Authorization header is missing."
        )
    ...
    if len(strs) != 2:
        raise ExceptionHandler(
            code=ErrorCodes.UNAUTHORIZED, msg="Invalid authorization header format."
        )

    tokenType = strs[0]
    if tokenType != "Bearer":
        raise ExceptionHandler(
            code=ErrorCodes.UNAUTHORIZED, msg="Ensure use of Bearer token."
        )

    accessToken = strs[1]
    if not accessToken:
        raise ExceptionHandler(
            code=ErrorCodes.UNAUTHORIZED, msg="Please provide a valid access token."
        )
...
async def auth_middleware(
    ...
    try:
        ...
        if not account:
            raise ExceptionHandler(
                code=ErrorCodes.UNAUTHORIZED, msg="Account not found in database"
            )
        ...
        raise ExceptionHandler(
            code=ErrorCodes.UNAUTHORIZED, msg=f"Unauthorized: {exception}"
        )
