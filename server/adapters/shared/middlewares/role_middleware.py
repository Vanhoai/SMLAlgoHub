...
def role_middleware(required: List[EnumRole] = []):
    async def execute(
        req: Request,
        role_service: RoleService = Depends(dependencies.role_service)
    )-> bool:
        if len(required) == 0:
            return True

        if not req.state.account:
            raise ExceptionHandler(code=ErrorCodes.NOT_FOUND, msg="Cannot find account in request")

        enitiy = await role_service.find_one(req.state.account.id)
        if not enitiy:
            raise ExceptionHandler(code=ErrorCodes.FORBIDDEN, msg="Not enough permissions")
